...
module.exports.Gen = function(_gameData, __) {
    ...
    `<div id="user-games">
        ...
        <div id="user-games-body" class="in-box">{"data": [`;
    _gameData.forEach(game => {
        ...
        // html +=
        // `<a class="user-games-game" href="${url}" target="_blank" rel="noopener noreferrer" timestamp="${timestamp}" gametype="${queueType}" champname="${__(CHAMPION[game.champion])}" champid="${CHAMPION[game.champion]}" lane="${LANES[game.lane_my]}">
        //     <img class="user-games-icon" src="${CHAMPIONURI + CHAMPION[game.champion]}.png">
        //     <span class="user-games-date">???</span><br>
        //     <span class="user-games-type">${__('icon_'+ queueType)}</span>
        // </a>
        // `;
        html += `["${Object.keys(PLATFORM_MY)[game.platform_my].toUpperCase()}/${game.game_id}", "${timestamp}", "${queueType}", "${CHAMPION[game.champion]}", "${LANES[game.lane_my]}"],`;
    })
    html = html.slice(0, -1);
    html += `]}</div></div>`
    return html;
