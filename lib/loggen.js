...
const { QUEUETYPE, CHAMPION, PLATFORM_MY, RIOTCDNURI } = require('./constant');
...
module.exports.Gen = function(_gameData, __) {
    ...
    _gameData.forEach(game => {
        ...
        html +=
        `<a class="user-games-game" href="${url}" target="_blank" rel="noopener noreferrer" timestamp="${timestamp}" gametype="${queueType}" champid="${__(CHAMPION[game.champion])}">
