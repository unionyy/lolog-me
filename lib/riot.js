...
module.exports.GetGame = async function (_matchId, _platform) {
    ...
    if(data.code === 200) {
        ...
        for(teamId in match.teams) {
            for(participant of match.teams[teamId].participants) {
                var userId = await dbio.GetUserAccountId(participant.id.accountId, _platform, 'id_my');
                if(userId) {
                    var detailGame = {
                        id_my:          userId.id_my,
                        game_id:        _matchId,
                        play_time:      match.play_time,
                        platform_my:    PLATFORM_MY[_platform],
                        champion:       participant.champ,
                        queue_type:     match.queue_type,
                        lane_my:        0, 

                        win_my:         match.teams[teamId].win,
                        kills:          participant.stats.kills,
                        deaths:         participant.stats.deaths,
                        assists:        participant.stats.assists,
                        total_kills:    match.teams[teamId].kills,

                        duration:       match.duration,
                        cs:             participant.stats.minions + participant.stats.jungle,
                        multi_kill:     participant.stats.multiKill,
        ...
        if(games.length > 0) {
            dbio.InputGameDetailMulti(games);
        }
...
async function SearchUserAPI (_normName, _platform, _isAccountId) {
...
//                 var rev = await SearchGamesAPI(newUserData.account_id, _platform, newUserData.explore_idx, new Date(new Date() - 31536000000), newUserData.id_my, 4);
