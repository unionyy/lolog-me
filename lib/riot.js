    ...
    if(userData) {
        ...
        if(apiUserData) {
            ...
            if(userDataPuuid) {
                ...
                await SearchGamesAPI(newUserData.account_id, _platform, newUserData.explore_idx, new Date(new Date() - 31536000000), newUserData.id_my, newUserData.puuid);
...
module.exports.SetDate = async function (_normName, _platform, _dateBegin, _dateEnd) { // Begin > End
    var userData = await dbio.GetUser(_normName, PLATFORM_MY[_platform]);
    if(userData) {
        await SearchGamesAPI(userData.account_id, _platform, userData.explore_idx, _dateEnd, userData.id_my, userData.puuid);
        if(SearchGamesAPI) {
