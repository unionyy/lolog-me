...
module.exports.SearchCustom = async function(_normName, _platform, _dateBegin, _dateEnd) {
    ...
    while(searchQ[_normName+_platform]) {
        console.log('waiting...');
    ...
    if(userData) {
        ...
        if(apiUserData) {
            ...
            if(userDataPuuid) {
                ...
                var rev = await SearchGamesAPI(newUserData.account_id, _platform, newUserData.explore_idx, _dateEnd, newUserData.id_my, 8);

                console.log("multi search done")
                if(rev && rev.complete) {
