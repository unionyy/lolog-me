...
module.exports.Gen = function (__) {
    ...
    for (var i = 0; i <= 366; i++) {
        ...
        var ly = day * 20;
        html += `<a onclick="$('#user-games-refresh').attr('date', '${cs}');Change();" style="cursor:pointer;"><rect id="rect-${cs}" class="day" title="???${__('game_count')}, ${cs}" width="16" height="16" rx="3" x=${lx} y=${ly} style="stroke: #e2e4e7; fill: #ebedf0" data-date=${cs}></rect></a>`


        // Month tag
        if(ctime.getDate() === 15) [
            monhtml += `<text x=${tx + lx - 5} y="-7" class="month">${__('month_' + ctime.getMonth())}</text>`
        ]
