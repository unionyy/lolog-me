...
module.exports.SearchMatches = async function(_idMy, _matchIds, _platform) {
    const matches = {};
    /** Get Matches From DB */
    const dbMatches = await dbio.GetMatches(_idMy, _matchIds) || {};
    for(const match of dbMatches) matches[match.match_id] = match;

    /** Get Matches From Riot API */
    const dbMissMatchIds = [];
    for(const matchId of _matchIds) if(!matches[matchId]) dbMissMatchIds.push(matchId);

    const matchPromises = {};
    const apiMatches = {};
    for(const matchId of dbMissMatchIds) matchPromises[matchId] = SearchMatchAPI(matchId, _platform);
    for(const matchId of dbMissMatchIds) {
        const apiMatch = await matchPromises[matchId];
        if(apiMatch) apiMatches[matchId] = apiMatch;
    }
    await dbio.InputMatchesWithParticipants(apiMatches, _platform);
    
    /** Get Participants From DB (Participants MUST Exist in DB */
    const dbParticipants = await dbio.GetParticipants(_idMy, _matchIds);
    for(const matchId of _matchIds) matches[matchId].participant = dbParticipants[matchId];
    for(const participant of dbParticipants) matches[participant.match_id]?.participant = participant;

    return matches;
}

async function SearchMatchAPI(_matchId, _platform) {
    const match = await riotApi.MatchV5.matches(_matchId, _platform);
    ...
    for (const participant of match.info.participants) {
        teams[participant.teamId].participants.push({
            ...
            summoner1Id:    participant.summoner1Id, // spell1
            summoner2Id:    participant.summoner2Id, // spell2
            runeMainId:     participant.perks.styles[0].selections[0].perk,
        ...
        teams[participant.teamId].goldEarned += participant.goldEarned;
        teams[participant.teamId].totalDamageDealtToChampions += participant.totalDamageDealtToChampions;
    }
