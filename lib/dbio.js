...
module.exports.GetUserAcountId = function (_accountId) {
    return new Promise((resolve, reject,) => {
        db.query(
            `SELECT id_my FROM users WHERE account_id=?`,
            [_accountId],
            (err, results) => {
                if(err) {
                    reject(err);
...
module.exports.InputGameDetailMulti = function (_games) {
    return new Promise((resolve, reject,) => {
        var queryString =`INSERT INTO games (id_my, game_id, play_time, platform_my, champion, queue_type, lane_my, win_my, kills, deaths, assists, total_kills) 
            VALUES`;
        var data = [];

        for(game of _games) {
            queryString += ` (?, ?, FROM_UNIXTIME(?), ?, ?, ?, ?, ?, ?, ?, ?, ?),`
            data.push(game.id_my, game.game_id, game.play_time, game.platform_my, game.champion, game.queue_type, game.lane_my, game.win_my, game.kills, game.deaths, game.assists, game.total_kills)
        }

        queryString = queryString.slice(0, -1);
        queryString +=  `AS new ON DUPLICATE KEY UPDATE win_my=new.win_my, kills=new.kills, deaths=new.deaths, assists=new.assists, total_kills=new.total_kills`;
