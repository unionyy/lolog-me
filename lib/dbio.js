...
module.exports.InputUser = function (_data) {
    return new Promise((resolve, reject,) => {
        db.query(
            ...
            VALUES(?, ?, NOW(), 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE norm_name=?, platform_my=?, update_time=NOW(), account_id=?, summoner_id=?, real_name=?, profile_icon_id=?, summoner_level=?, solo_tier=?, solo_rank=?, solo_lp=?, solo_wins=?, solo_losses=?, flex_tier=?, flex_rank=?, flex_lp=?, flex_wins=?, flex_losses=?`,
            [_data.norm_name, _data.platform_my, _data.account_id, _data.summoner_id, _data.puuid, _data.real_name, _data.profile_icon_id, _data.summoner_level, _data.solo_tier, _data.solo_rank, _data.solo_lp, _data.solo_wins, _data.solo_losses, _data.flex_tier, _data.flex_rank, _data.flex_lp, _data.flex_wins, _data.flex_losses, _data.norm_name, _data.platform_my, _data.account_id, _data.summoner_id, _data.real_name, _data.profile_icon_id, _data.summoner_level, _data.solo_tier, _data.solo_rank, _data.solo_lp, _data.solo_wins, _data.solo_losses, _data.flex_tier, _data.flex_rank, _data.flex_lp, _data.flex_wins, _data.flex_losses],
            (err, results) => {
                if(err) {
                    reject(err);
                } else {
...
module.exports.GetGames = function (normname) {
    return new Promise((resolve, reject,) => {
        db.query(
            `SELECT * FROM games WHERE normname=?`,
            [normname],
            (err, results) => {
                if(err) {
                    reject(err);
                    ...
                    console.log(`select games: ${normname}`);
                    resolve(false);
                }
            });
    });
