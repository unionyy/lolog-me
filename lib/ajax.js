module.exports.Match = function (_origin) {
    ...
    var bestDealer = [-1, -1, 0];
    var bestKillPart = [-1, -1, 0];
    var bestVison = [-1, -1, 0];
    ...
    for (elem of _origin.participants) {
        var participant = {
            ...
            stats: {
                ...
                rune0: elem.stats.perk0 || 0,
                rune1: elem.stats.perkSubStyle || 0,
                ...
                visionScore: elem.stats.visionScore || 0,
                wardsBought: elem.stats.visionWardsBoughtInGame || 0,
                wardsPlaced: elem.stats.wardsPlaced || 0,
                wardsKilled: elem.stats.wardsKilled || 0
            },
            bestPlayer: 0
        }
        ...
        if(participant.stats.deal > bestDealer[2]) {
            bestDealer[0] = elem.teamId;
            bestDealer[1] = teams[elem.teamId].participants.length - 1;
            bestDealer[2] = participant.stats.deal;
        }
        if(participant.stats.kills + participant.stats.assists > bestKillPart[2]) {
            bestKillPart[0] = elem.teamId;
            bestKillPart[1] = teams[elem.teamId].participants.length - 1;
            bestKillPart[2] = participant.stats.kills + participant.stats.assists;
        }
        if(participant.stats.visionScore > bestVison[2]) {
            bestVison[0] = elem.teamId;
            bestVison[1] = teams[elem.teamId].participants.length - 1;
            bestVison[2] = participant.stats.visionScore;
        }
    }

    if(bestDealer[0] != -1) {
        teams[bestDealer[0]].participants[bestDealer[1]].bestPlayer += 1;
    }
    if(bestKillPart[0] != -1) {
        teams[bestKillPart[0]].participants[bestKillPart[1]].bestPlayer += 2;
    }
    if(bestVison[0] != -1) {
        teams[bestVison[0]].participants[bestVison[1]].bestPlayer += 4;
    }
